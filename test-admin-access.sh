...
echo "Testing top-level access (should work)..."
aws s3 ls s3://kyndly-ichra-documents/submissions/

echo "Testing TPA-level access (should work)..."
aws s3 ls s3://kyndly-ichra-documents/submissions/test-tpa-123/

echo "Testing employer-level access (should work)..."
aws s3 ls s3://kyndly-ichra-documents/submissions/test-tpa-123/test-employer-456/

echo "Testing TPA file access (should work)..."
aws s3 cp s3://kyndly-ichra-documents/submissions/test-tpa-123/test-file.txt -

echo "Testing employer file access (should work)..."
aws s3 cp s3://kyndly-ichra-documents/submissions/test-tpa-123/test-employer-456/test-file.txt -
