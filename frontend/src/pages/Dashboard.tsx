...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-teal-800 to-blue-900">
      {/* Main Banner */}
      <div className="bg-brand-gradient rounded-brand p-6 mb-8 text-white shadow-brand border border-[#46BC97]/40">
      ...
      <div className="p-6 space-y-6">
        ...
        >
          <motion.div
            ...
            className={`${theme.card} border border-slate-200 dark:border-[#46BC97]/40 p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 dark:bg-[#042C3A]/90`}
          >
            <div className="flex items-center justify-between">
              ...
              <div className={`p-3 rounded-xl bg-teal-50 dark:bg-teal-900/20 shadow-sm`}>
                <DocumentTextIcon className="h-7 w-7 text-teal-600 dark:text-white" aria-hidden="true" />
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className={`p-3 rounded-xl bg-teal-50 dark:bg-teal-900/20 shadow-sm`}>
                <DocumentIcon className="h-7 w-7 text-teal-600 dark:text-white" aria-hidden="true" />
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className={`p-3 rounded-xl bg-teal-50 dark:bg-teal-900/20 shadow-sm`}>
                <ArrowUpTrayIcon className="h-7 w-7 text-teal-600 dark:text-white" aria-hidden="true" />
              </div>
            </div>
          </motion.div>
        </motion.div>
        ...
        <motion.div
          ...
          className={`${theme.card} border border-slate-200 dark:border-[#46BC97]/40 p-4 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 dark:bg-[#042C3A]/90`}
        >
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm font-medium text-teal-600 dark:text-white">
                ...
                <div className={`p-3 rounded-xl bg-green-50 dark:bg-green-900/20 shadow-sm`}>
                  <CheckCircleIcon className="h-7 w-7 text-green-600 dark:text-white" aria-hidden="true" />
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 shadow-sm`}>
                  <ChartBarIcon className="h-7 w-7 text-blue-600 dark:text-white" aria-hidden="true" />
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex flex-col">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className={`p-2 rounded-xl bg-amber-50 dark:bg-amber-900/20 shadow-sm`}>
                    <ClockIcon className="h-6 w-6 text-amber-600 dark:text-white" aria-hidden="true" />
                  </div>
                </div>
                ...
                <div className="space-y-2 mt-2">
                  {dashboardData.statusCounts ? (
                    Object.entries(dashboardData.statusCounts).map(([status, count]) => (
                      <div key={status} className="flex justify-between items-center p-2 hover:bg-slate-50 dark:hover:bg-[#46BC97]/10 rounded-lg transition-colors duration-200">
                        <span className="capitalize font-medium text-slate-700 dark:text-white">{status}</span>
                        <span className="font-bold text-slate-800 dark:text-white">{count}</span>
                      </div>
                    ))
                  ) : (
                    <div className="text-center text-slate-500 dark:text-slate-400 p-2 rounded-lg bg-slate-50 dark:bg-[#031C25] dark:border dark:border-[#46BC97]/20">No data available</div>
                  )}
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex flex-col">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className={`p-2 rounded-xl bg-purple-50 dark:bg-purple-900/20 shadow-sm`}>
                    <DocumentPlusIcon className="h-6 w-6 text-purple-600 dark:text-white" aria-hidden="true" />
                  </div>
                </div>
                ...
                <div className="space-y-2 mt-2">
                  {dashboardData.topBrokers && dashboardData.topBrokers.length > 0 ? (
                    dashboardData.topBrokers.map((broker: {name: string, count: number}) => (
                      <div key={broker.name} className="flex justify-between items-center p-2 hover:bg-slate-50 dark:hover:bg-[#46BC97]/10 rounded-lg transition-colors duration-200">
                        <span className="font-medium dark:text-white">{broker.name}</span>
                        <span className="font-bold text-slate-800 dark:text-white">{broker.count} quotes</span>
                      </div>
                    ))
                  ) : (
                    <div className="text-center text-slate-500 dark:text-slate-400 p-2 rounded-lg bg-slate-50 dark:bg-[#031C25] dark:border dark:border-[#46BC97]/20">No broker data available</div>
                  )}
                </div>
              </div>
            </motion.div>
        ...
        <motion.div
          ...
          className={`${theme.card} border border-slate-200 dark:border-[#46BC97]/40 p-6 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 dark:bg-[#042C3A]/90`}
        >
          ...
          <div className="mt-4 space-y-4">
            {dashboardData.recentQuotes && dashboardData.recentQuotes.length > 0 ? (
              <ul className="divide-y divide-slate-200 dark:divide-[#031C25]">
                  ...
                  >
                    <motion.div
                      ...
                      className={`p-2 rounded-lg ${
                        (quote.status?.toLowerCase() === 'active' || quote.status?.toLowerCase() === 'approved') ? 'bg-teal-50 dark:bg-teal-900/20' : 
                        quote.status?.toLowerCase() === 'pending' ? 'bg-amber-50 dark:bg-amber-900/20' : 
                        quote.status?.toLowerCase() === 'inactive' ? 'bg-gray-50 dark:bg-gray-800' : 'bg-red-50 dark:bg-red-900/20'
                      } shadow-sm`}
                    >
                      <span className="font-medium dark:text-white">{quote.name || quote.companyName}</span>
                    </motion.div>
              ...
              <div className="px-4 py-5 text-center text-sm text-slate-500 dark:text-slate-400 rounded-lg bg-slate-50 dark:bg-[#031C25] dark:border dark:border-[#46BC97]/20">
